...
func (s Service) EnforceRole(c echo.Context, r jobsity.AccessRole) error {
	return checkBool(!(c.Get("role").(jobsity.AccessRole) > r))
}
...
func (s Service) EnforceCompany(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, jobsity.CompanyAdminRole); err != nil {
		return err
	}
...
func (s Service) EnforceLocation(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, jobsity.LocationAdminRole); err != nil {
		return err
	}
...
func (s Service) isAdmin(c echo.Context) bool {
	return !(c.Get("role").(jobsity.AccessRole) > jobsity.AdminRole)
}

func (s Service) isCompanyAdmin(c echo.Context) bool {
	// Must query company ID in database for the given user
	return !(c.Get("role").(jobsity.AccessRole) > jobsity.CompanyAdminRole)
}
...
func (s Service) AccountCreate(c echo.Context, roleID jobsity.AccessRole, companyID, locationID int) error {
...
func (s Service) IsLowerRole(c echo.Context, r jobsity.AccessRole) error {
	return checkBool(c.Get("role").(jobsity.AccessRole) < r)
}
