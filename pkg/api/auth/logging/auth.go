...
func New(svc auth.Service, logger jobsity.Logger) *LogService {
	return &LogService{
		Service: svc,
		logger:  logger,
	}
...
func (ls *LogService) Authenticate(c echo.Context, user, password string) (resp jobsity.AuthToken, err error) {
...
// Me logging
func (ls *LogService) Me(c echo.Context) (resp jobsity.User, err error) {
	defer func(begin time.Time) {
		ls.logger.Log(
			c,
			name, "Me request", err,
