...
import (
	...
	"my-chat-jobsity-challenge/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID jobsity.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr jobsity.User) (jobsity.User, error) {
					usr.ID = 1
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *jobsity.ListQuery, p jobsity.Pagination) ([]jobsity.User, error) {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (jobsity.User, error) {
					return jobsity.User{
						Role: &jobsity.Role{
							AccessLevel: jobsity.CompanyAdminRole,
						},
					}, nil
				},
			},
