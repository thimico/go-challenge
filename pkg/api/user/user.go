...
func (u User) Create(c echo.Context, req jobsity.User) (jobsity.User, error) {
	if err := u.rbac.AccountCreate(c, req.RoleID, req.CompanyID, req.LocationID); err != nil {
		return jobsity.User{}, err
	}
...
func (u User) List(c echo.Context, p jobsity.Pagination) ([]jobsity.User, error) {
	au := u.rbac.User(c)
	q, err := query.List(au)
	if err != nil {
		return nil, err
...
func (u User) View(c echo.Context, id int) (jobsity.User, error) {
...
func (u User) Update(c echo.Context, r Update) (jobsity.User, error) {
	if err := u.rbac.EnforceUser(c, r.ID); err != nil {
		return jobsity.User{}, err
	}

	if err := u.udb.Update(u.db, jobsity.User{
		Base:      jobsity.Base{ID: r.ID},
